...
abstract class ContentElement extends \Frontend
{
	...
	public function generate()
	{
		...
		//FTC Classes 
		$ftc_align= $this->splitArr($this->align_ftc);
		$ftc_data_attr = $this->splitArr($this->data_attr_ftc);
		$ftc_classes = $this->small_ftc.' '.$this->large_ftc.' '.$this->float_ftc.' '.$ftc_align.' columns';
		$this->Template->data_attr = $ftc_data_attr;
		$this->Template->ftc_classes = $ftc_classes;
		$this->Template->class = trim('ce_' . $this->type . ' ' . $this->cssID[1]);
		$this->Template->cssID = ($this->cssID[0] != '') ? ' id="' . $this->cssID[0] . '"' : '';
		//$this->Template->test = $this->arrData;
