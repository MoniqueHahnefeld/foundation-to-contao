...
class PaginationFTC
{
	 ...
	 */
	public function __construct($intRows, $intPerPage, $intNumberOfLinks=7, $strParameter='slide', \Template $objTemplate=null)
	{
		$this->intPage = 1;
		...
		if ($objTemplate === null)
		{
			$objTemplate = new \FrontendTemplate('pagination_ftc');
		}
	...
	public function getItemsAsString($strSeparator=' ')
	{
		...
		for ($i=$intFirstLink; $i<=$intLastLink; $i++)
		{
			if ($i == $this->intPage)
			{
				$arrLinks[] = sprintf('<li class="current"><a href="#" >%s</a></li>', $i);
				continue;
			}
